...
function test_example()
    ...
    Mz_est, kdests = fit_marginals(My, SJ())

    #T = 5_000
    T = size(df, 1) # number of observations
    p = 23          # order of process
    ...
    F = hcat(
        ...
        create_dummy_encoding(df[p+1:T,:hour], 0:23),
        create_dummy_encoding(df[p+1:T,:month], 1:12)
    )
    ...
    chain = reduce(chainscat, chains_per_thread)

    plot(ess(chain).nt.ess)
    minimum(ess(chain).nt.ess)
    argmin(ess(chain).nt.ess)
    plot(log.(ess(chains).nt.ess))

    plot(posterior.μ)
    plot(mean(chain).nt.mean)

    i = 150
    plot(chain, Symbol("log_ξ[149]"))
    plot(chain, Symbol("β[1]"))

    plot(chain, Symbol("log_τ[1]"))
    plot(chain, Symbol("γ[1]"))

    mapslices(std, chain.value.data[:,:,1], dims=1)
