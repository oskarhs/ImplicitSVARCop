...
function sample_mh_τ_ars(rng::Random.AbstractRNG, log_τ::AbstractVector, log_ξ::AbstractVector, J::Int, K::Int)
    log_τ_new = Vector{Float64}(undef, K)
    for k = 1:K
        log_τ_new[k] = sample_mh_τ_k(rng, log_τ[k], log_ξ[(k-1)*J+1:k*J], J, K)
    end
    return log_τ_new
end