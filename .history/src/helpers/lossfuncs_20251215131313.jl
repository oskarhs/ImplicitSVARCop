...
"""
    rmse(rng::Random.AbstractRNG, y_obs::AbstractVector{<:Real}, posterior_samples, kdests, model, p, H)

Get the RMSE of h-step ahead forecasts for h in 1:H
"""
function rmse(rng::Random.AbstractRNG, y_obs::AbstractVector{<:Real}, posterior_samples, kdests, model, p, H)
    mean_pred = predict_means(rng, posterior_samples, kdests, model, p, H)

    rmse_vec = Array{Float64}(undef, (K, H))
    for k in 1:K
        for h in 1:H
            rmse_vec[k] = sqrt(sum((y_obs[:, k, h] - mean_pred[:, k, h]).^2))
        end
    end
    return rmse_vec
end