...
function example_normaldata()
    ...
    sampler_ξ = NUTS(0.75)
    sampler_γ = NUTS(0.75)
    samples = Vector{Vector{Vector{Float64}}}(undef, 2)

    samples[1] = composite_gibbs_abstractmcmc_lkj(rng, model, sampler_ξ, sampler_γ, θ_init, n_samples; n_adapts=n_adapts, progress=true)
    samples[2] = composite_gibbs_abstractmcmc_lkj(rng, model, sampler_ξ, sampler_γ, θ_init, n_samples; n_adapts=n_adapts, progress=true)
    #chain = Chains(samples[n_adapts+1:end], ImplicitSVARCop.get_varsymbols_lkj(model))

    chains_per_thread = map(samples -> MCMCChains.Chains(samples, ImplicitSVARCop.get_varsymbols_lkj(model)), samples)
    combined_chains = reduce(chainscat, chains_per_thread)
